...
$_SESSION['reserva']['tipo_voo']=$tipo_voo;
$_SESSION['reserva']['data_ida']=$data_ida;
$_SESSION['reserva']['data_volta']=$data_volta;
?>
...
function cria_tab($res, $usu_origem, $usu_destino) {
...
// Cria tabela de desitâncias.
 	 ...
 	 $hora_chegada=calc_chegada($hora_partida,$tempo);
	 ...
	 $nome_origem=$origem . "_o_" . $cont_origem[$origem];
		 
	 $HP[$nome_origem]=$hora_partida;
 ...
 //
	if(isset($cont_destino[$destino])){
	   $cont_destino[$destino]++;
	} else {
	   $cont_destino[$destino]=1;
	}	 
		 
	$nome_destino = $destino . "_d_" . $cont_destino[$destino];
		 
	$HC[$nome_destino]=$hora_chegada;
	$DES[$nome_destino]=1;
		 
	$T[$nome_origem][$nome_destino]=$tempo_voo;
		 

	if (mesmo_local($usu_origem,$nome_origem)) {
	   $T[$usu_origem][$nome_origem]=0;
	} else {
	   $T[$usu_origem][$nome_origem]=10000;
	}

	if (mesmo_local($usu_destino,$nome_destino)) {
	   $T[$nome_destino][$usu_destino]=0;
	} else {
	   $T[$nome_destino][$usu_destino]=10000;
        }

	  		 
        $cidade[$nome_origem]=1;
        $cidade[$nome_destino]=1;

        $dados_voo[$nome_origem] = 
		           $id_voo . ", " . 
		           $id_voo_data . "," .
				   $origem . ", " .  
				   $destino . ", " . 
...
if ($nr==0){
 ...
 cria_tab($result, $usu_origem, $usu_destino);
...
echo "<td rowspan='2'><p>".($t-1)."</p></td>";
 ...
 $_SESSION["reserva"]["tarifa_total_ida"] = $soma_preco;
 $_SESSION["reserva"]["cont_escala_ida"] = $t;
 $_SESSION["reserva"]["origem_".$i."_ida"]=$c1;
 $_SESSION["reserva"]["destino_".$i."_ida"]=$c2;
 $_SESSION["reserva"]["voo_".$i."_ida"]=$id;
 $_SESSION["reserva"]["voo_".$i."_dataida"]=$id_data;
 $_SESSION["reserva"]["hora_partida_".$i."_ida"]=$hora_partida;
 $_SESSION["reserva"]["hora_chegada_".$i."_ida"]=$hora_chegada;
...
else
{
 	...
 	cria_tab($result2,$usu_destino,$usu_origem);

 	$S = rota($usu_destino,$usu_origem);
	$t = count($S);

	echo "<tr><td rowspan='2' valign='top'><input type='radio' name='volta' onclick=\"valida_form('ver_preco.php')\"></td>";
	echo "<td rowspan='2' valign='top'><p>".(count($S)-1)."</p></td>";
       ...
       $hora_chegada=calc_chegada($hora_partida,$tempo_voo);
         ...
         echo "<td><input type='hidden' name='origem' value='$c1'><p> $c1<br>
              (".substr($hora_partida,0,5) .")</p></td>
               ...
               (".substr($hora_chegada,0,5).")</p></td>
                       <td><p>$aeronave</p></td>
                       <td><p>$id</p></td>
               <td><select name='classe_volta_$i'>";
                       if($qtde_total<$classe1){
		echo "<option  value='1classe'>1<sup>a</sup> Classe</option>";
		}
		if($qtde_total<$executiva){
		...
		echo "<option  value='economica'>Econômica</option>";
                      ...
                      echo "</select></td></tr>";
                       ...
                       $_SESSION["reserva"]["tarifa_total_volta"] = $soma_preco;
 $_SESSION["reserva"]["cont_escala_volta"] = $t;
 $_SESSION["reserva"]["origem_".$i."_volta"]=$c1;
 $_SESSION["reserva"]["destino_".$i."_volta"]=$c2;
 $_SESSION["reserva"]["voo_".$i."_volta"]=$id;
 $_SESSION["reserva"]["voo_".$i."_datavolta"]=$id_data;
 $_SESSION["reserva"]["hora_partida_".$i."_volta"]=$hora_partida;
 $_SESSION["reserva"]["hora_chegada_".$i."_volta"]=$hora_chegada;
